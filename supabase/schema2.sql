...
create or replace function public.set_disclaimer(p_enabled boolean, p_img text, p_text_fr text, p_text_en text, p_show_stats boolean, p_frequency text, p_expires_at timestamptz) returns void
language plpgsql security definer set search_path = public as $$
begin
  if not public.is_admin() then raise exception 'Admins only.'; end if;
...
grant execute on function public.set_disclaimer(boolean, text, text, text, boolean, text, timestamptz) to authenticated;
