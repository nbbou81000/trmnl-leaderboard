...
-- ---------- Bandeau d'accueil (disclaimer), modifiable par un administrateur depuis le site
...
-- Mise à niveau (au cas où la table existait déjà avant l'ajout de ces réglages)
alter table public.site_settings add column if not exists show_stats boolean not null default false;
alter table public.site_settings add column if not exists frequency text not null default 'day';
alter table public.site_settings add column if not exists expires_at timestamptz;
alter table public.site_settings add column if not exists font text not null default 'default';
alter table public.site_settings add column if not exists color text not null default '';
alter table public.site_settings drop constraint if exists site_settings_frequency_check;
alter table public.site_settings add constraint site_settings_frequency_check check (frequency in ('day', 'week', 'always'));

-- Seul un administrateur peut modifier le bandeau ; la version augmente à chaque enregistrement
-- pour que le bandeau réapparaisse chez tout le monde même s'il a déjà été vu depuis la dernière modification.
-- « img » peut contenir une URL externe (https://…) ou le chemin d'une image importée dans le stockage (bucket "wip").
drop function if exists public.set_disclaimer(boolean, text, text, text);
drop function if exists public.set_disclaimer(boolean, text, text, text, boolean, text, timestamptz);
create or replace function public.set_disclaimer(p_enabled boolean, p_img text, p_text_fr text, p_text_en text, p_show_stats boolean, p_frequency text, p_expires_at timestamptz, p_font text, p_color text) returns void
language plpgsql security definer set search_path = public as $$
begin
  if not public.is_admin() then raise exception 'Admins only.'; end if;
  update public.site_settings set
    ...
    img = coalesce(p_img, ''),
    text_fr = coalesce(p_text_fr, ''),
    text_en = coalesce(p_text_en, ''),
    show_stats = coalesce(p_show_stats, false),
    frequency = case when p_frequency in ('day', 'week', 'always') then p_frequency else 'day' end,
    expires_at = p_expires_at,
    font = coalesce(nullif(btrim(p_font), ''), 'default'),
    color = coalesce(p_color, ''),
    version = version + 1,
    updated_at = now()
  where id = 1;
end $$;
grant execute on function public.set_disclaimer(boolean, text, text, text, boolean, text, timestamptz, text, text) to authenticated;
...
-- Aucune policy insert/update/delete : les modifications passent uniquement par set_disclaimer() (security definer).
...
update public.profiles pr set name = left(coalesce(nullif(btrim(u.raw_user_meta_data->'custom_claims'->>'global_name'), ''), nullif(btrim(u.raw_user_meta_data->>'full_name'), ''),
    nullif(btrim(u.raw_user_meta_data->>'name'), ''), nullif(btrim(u.raw_user_meta_data->>'user_name'), ''), nullif(btrim(u.raw_user_meta_data->>'preferred_username'), ''), 'Anonymous'), 40)
  from auth.users u where u.id = pr.id and pr.name = '';
